...
local years 2015 2016 2017
 foreach year in `years' {
	...
	drop age
	gen imd = ""
	replace imd = "5_scot" if sqimd == 1 //the imd variable used in this analysis from the scottish health survey is coded the opposite way around to the English one, so change to match the english one, 1= least deprived, 5= most deprived.
	replace imd = "4_scot" if sqimd == 2
	replace imd = "3_scot" if sqimd == 3
	replace imd = "2_scot" if sqimd == 4
	replace imd = "1_scot" if sqimd == 5
	drop sqimd
...
gen only_lottery_yes = (only_lottery == "yes")

* Keep only the lottery-only group temporarily
preserve
keep if only_lottery_yes == 1

* Run the table
//dtable i.sex i.age_group i.qimd i.nssec8 i.mental_disorder i.alcohol_freq i.gam_freq gam_freq_c2 pgsisc, ///
   // by(pgsi_cat, tests) export(Tables\TableLotteryOnly.docx)
